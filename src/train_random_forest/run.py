...
def go(args):
    ...
    ######################################
    # Use run.use_artifact(...).file() to get the train and
    # validation artifact (args.trainval_artifact)
    ...
    # see: https://mlflow.org/docs/latest/python_api 
    #   /mlflow.sklearn.html?highlight=save_model#mlflow.sklearn.save_model
    
    # Without input data the following user warning was thrown during mlflow run:
    # .../mlflow/models/signature.py:130: UserWarning:
    # Hint: Inferred schema contains integer column(s). Integer columns in Python
    # cannot represent missing values. If your input data contains missing values
    # at inference time, it will be encoded as floats and will cause a schema
    # enforcement error. The best way to avoid this problem is to infer the model
    # schema based on a realistic data sample (training dataset) that includes
    # missing values. Alternatively, you can declare integer columns as doubles (float64)
    # whenever these columns may have missing values.
    # Therefore the input_example param is added ... but it happened again, 
    # so, imputation for inference pipeline is really important!
    ...
    # Use args.output_artifact as artifact name, "model_export" as type,
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    # for example for room type: 'Entire home/apt' > 'Private room' > 'Shared room'
    ...
    # 1 - a step called "preprocessor" applying the ColumnTransformer instance that we saved
    # in the `preprocessor` variable, and
    # 2 - a step called "random_forest" with the random forest instance that we just saved
    # in the `random_forest` variable.
    # HINT: Use the explicit Pipeline constructor so you can assign the names to the steps,
    # do not use make_pipeline
    sk_pipe = Pipeline(
        steps = [
